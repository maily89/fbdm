		...
		private void SaveToDatabase_withDataSet()
		{
			try
			{
				if (fileCheck())
				{
					...
					CreateTableInDatabase(dt, this.txtOwner.Text, this.txtTableName.Text, prop.sqlConnImportString);

					// Copies all rows to the database from the dataset.
					using (SqlBulkCopy bc = new SqlBulkCopy(prop.sqlConnImportString))
					{
