...
<table>
		...
		<td><input type="button" value="Basic Info"  class="<%=Model==FBD.CommonUtilities.Constants.IndividualRankStep.Basic?"currentStep":"step"%>" onclick="window.location.href='<%= Url.Action("DetailBasic", new { id = Url.RequestContext.RouteData.Values["id"] } ) %>';"/></td>		
